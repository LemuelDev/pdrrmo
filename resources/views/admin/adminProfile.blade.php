    ...
    <section class="min-h-[100vh]  flex items-start">
        ...
        <div class="flex-grow pt-8 h-auto">
            ...
            <div class="pt-4 pl-8 max-sm:text-center max-sm:pl-0">
                <h4 class="text-2xl font-bold pt-2 ">ADMIN PROFILE</h4>
            ...
            <div class=" grid content-center px-8 max-[430px]:px-4 xl:max-w-[1500px] " >
                @if ($editing ?? false)
                    <form action="{{route('admin.update', auth()->user()->userProfile->id)}}" method="post" class="grid grid-cols-2 gap-8 px-8 pt-8 max-md:px-4 max-sm:grid-cols-1 max-sm:pb-4" enctype="multipart/form-data">
                         @csrf
                         @method('put')
                        <div class="grid">
                        ...
                        <div class="flex justify-start gap-4 max-lg:grid max-lg:content-start max-sm:max-w-[250px] max-sm:mx-auto">
                            <a href="{{route('admin.showRequest')}}" class="py-2 pt-3 text-lg px-7 max-xl:px-4 mt-10 max-lg:mt-0  max-lg:text-md text-center border-r-full border-none bg-purple-500 hover:bg-purple-600 text-white rounded-lg  ">Transfer of Municipality</a>
                            <a href={{ route('admin.password', ['user' => auth()->user()->id]) }} class="py-2 pt-3 max-xl:px-4 text-lg max-lg:mt-0  max-lg:text-md  px-7 mt-10 text-center border-r-full border-none bg-gray-600 hover:bg-gray-700 text-white rounded-lg ">Update Password</a>
                        </div>
                ...
                <div class="flex justify-evenly gap-4 items-start pt-4 max-md:gap-8 max-sm:gap-4 max-sm:grid">
                    <div class="grid text-center max-sm:max-w-[200px] max-sm:mx-auto max-sm:h-[auto]">
                        ...
                        <div class="pt-8">
                            <a href="{{route('admin.edit', auth()->user()->userProfile->id)}}" class="py-3 px-12 mt-4 text-white border-none rounded-md shadow-xl bg-green-500 hover:bg-green-600 max-w-[130px] text-lg hover:no-underline"> EDIT </a>
                        </div>
                    </div>

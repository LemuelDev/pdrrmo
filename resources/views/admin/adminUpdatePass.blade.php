           ...
           <div class="py-4 pl-8 flex items-start justify-around max-w-[1200px] gap-4 max-md:p-4 max-md:flex-col-reverse max-md:px-4">
             <!-- Form Container -->
                <div class="max-w-[500px] w-full max-md:max-w-[700px] max-md:mx-auto max-sm:w-full">
                    <form action="{{ route('admin.passUpdate', auth()->user()->id) }}" method="post" class="grid gap-6 p-8 px-10 w-full shadow-2xl rounded-lg max-md:min-w-0 max-md:p-6 max-sm:px-4 max-sm:w-full" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="grid py-2">

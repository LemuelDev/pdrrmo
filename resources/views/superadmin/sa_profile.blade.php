    ...
    <section class="min-h-[100vh]  flex items-start">
        ...
        <div class="flex-grow pt-8 h-auto">
            ...
            <div class="pt-4 pl-8 max-sm:text-center max-sm:pl-0">
                <h4 class="text-2xl font-bold pt-2 ">SUPERADMIN PROFILE</h4>
            ...
            <div class=" grid content-center px-8 max-[430px]:px-4 xl:max-w-[1500px] " >
                @if ($editing ?? false)
                    <form action="{{route('sa.update', auth()->user()->userProfile->id)}}" method="post" class="grid grid-cols-2 gap-8 px-8 pt-8 max-md:px-4 max-sm:grid-cols-1 max-sm:pb-4" enctype="multipart/form-data">
                         @csrf
                         @method('put')
                        <div class="grid">
                        ...
                        <div class="flex justify-start gap-4 max-lg:grid max-lg:content-start max-sm:max-w-[250px] max-sm:mx-auto">
                            {{-- <a href="{{route('sa.showRequest')}}" class="py-2 pt-3 text-lg px-7 max-xl:px-4 mt-10 max-lg:mt-0  max-lg:text-md text-center border-r-full border-none bg-purple-500 hover:bg-purple-600 text-white rounded-lg  ">Transfer of Municipality</a> --}}
                            <a href={{ route('sa.password', ['user' => auth()->user()->id]) }} class="py-2 pt-3 max-xl:px-4 text-lg max-lg:mt-0  max-lg:text-md  px-7 mt-10 text-center border-r-full border-none bg-gray-600 hover:bg-gray-700 text-white rounded-lg ">Update Password</a>
                        </div>

...
@section('content')
    <section class="min-h-[100vh] w-full flex items-start">
        ...
        <div class="flex-grow px-4 pt-8 h-auto border-solid max-sm:pt-4">
           ...
           <div class="py-4 pl-8 flex items-start justify-around max-w-[1200px] gap-4 max-md:p-4 max-md:flex-col-reverse max-md:px-4 max-sm:px-0">
            <!-- Form Container -->
               <div class="max-w-[500px] w-full max-md:max-w-[700px] max-md:mx-auto max-sm:w-full">
                   <form action="{{ route('staff.passUpdate', auth()->user()->id) }}" method="post" class="grid gap-6 p-8 px-10 w-full shadow-2xl rounded-lg max-md:min-w-0 max-md:p-6 max-sm:px-4 max-sm:w-full" enctype="multipart/form-data">
                       @csrf
                       @method('put')
                       <div class="grid py-2">
                           <label for="old_password" class="text-xl font-bold pb-4">CURRENT PASSWORD:</label>
                           <input type="password" name="old_password" id="old_password" placeholder="Enter current password" class="py-3  px-6 rounded-lg shadow-xl bg-slate-100  outline-green-500">
                       </div>
                       <div class="grid py-2">
                           <label for="new_password" class="text-xl font-bold pb-4">NEW PASSWORD:</label>
                           <input type="password" name="new_password" id="new_password" placeholder="Enter new password" class="py-3  px-6 rounded-lg shadow-xl bg-slate-100  outline-green-500">
                       </div>
                       <div class="grid py-2">
                           <label for="new_password_confirmation" class="text-xl font-bold pb-4">CONFIRM NEW PASSWORD:</label>
                           <input type="password" name="new_password_confirmation" id="new_password_confirmation" placeholder="Confirm new password" class="py-3  px-6 rounded-lg shadow-xl bg-slate-100  outline-green-500">
                       </div>
                       <a href="{{route('user.resetPassword')}}" class="text-blue-600 text-center text-lg hover:underline">Forgot password?</a>
                       <button type="submit" class="py-3 text-xl px-7 bg-green-500 hover:bg-green-600 text-white rounded-lg">UPDATE</button>
                   </form>
                   @if ($errors->any())
                   <div class="grid mt-7">

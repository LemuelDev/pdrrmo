...
<section class="flex items-center justify-center w-full h-full py-4">
    <div class="grid grid-cols-1 md:grid-cols-2  max-w-[1100px]  h-[650px] m-auto p-6 pt-36">
        ...
        <form action="{{route('password.email')}}" method="post" class="grid content-center gap-5 py-14 px-12 shadow-2xl rounded-xl">
            @csrf
            <h4 class="text-center text-black py-4 text-3xl font-bold">FORGOT PASSWORD</h4>
            <p>Kindly enter your email and we will send you a reset password link to reset and create your new password.</p>
            ...
            <button type="submit" class="py-3 text-xl px-7 border-r-full border-none bg-green-500 text-white rounded-lg">Send Rest Password Link</button>
